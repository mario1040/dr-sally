        ...
        <div className="grid lg:grid-cols-12 gap-12 lg:gap-20 items-center">
          ...
          >
            ...
            <motion.h2 variants={fadeUp} className="text-2xl md:text-3xl font-medium text-transparent bg-clip-text bg-gradient-to-r from-amber-600 to-amber-500 mb-8 font-cairo">
               {isRTL ? 'أخصائية الجلدية والتجميل والليزر' : 'Dermatology, Aesthetics & Laser Specialist'}
            </motion.h2>

            {/* نبذة عن الدكتورة */}
            <motion.p variants={fadeUp} className="text-lg text-slate-500 leading-relaxed mb-10 max-w-2xl font-medium">
               {isRTL 
                  ? 'متخصصة في الجلدية والتجميل والليزر، بتعتمد في شغلها على الجمع بين التشخيص الدقيق والخبرة العملية واختيار الإجراء المناسب لكل حالة. بتؤمن إن التجميل مش معناه تغيير الملامح، لكن الوصول لنتيجة طبيعية ومتناسقة، مع الحفاظ على شخصية كل حالة وملامحها.'
                  : 'A specialist in dermatology, aesthetics, and laser, she relies on combining accurate diagnosis with practical experience to choose the most suitable procedure for each case. She believes that aesthetics does not mean changing one\'s features, but rather achieving a natural and harmonious result while preserving the unique personality and features of every individual.'}
            </motion.p>
            ...
            <motion.div variants={fadeUp} className="grid sm:grid-cols-2 gap-4 w-full max-w-2xl mb-12">
               <div className="p-5 rounded-2xl bg-white border border-slate-100 shadow-sm flex items-start gap-4 group hover:border-amber-200 transition-colors">
                  ...
                  <div>
                     <h4 className="text-slate-900 font-bold mb-1 font-cairo">{isRTL ? 'المؤهلات العلمية' : 'Qualifications'}</h4>
               ...
               <div className="p-5 rounded-2xl bg-white border border-slate-100 shadow-sm flex items-start gap-4 group hover:border-amber-200 transition-colors">
                  ...
                  <div>
                     ...
                     <p className="text-sm text-slate-500 font-medium">{isRTL ? 'المعهد القومي لعلوم الليزر، جامعة القاهرة' : 'National Institute of Laser Sciences, Cairo University'}</p>
                  </div>
               </div>
            </motion.div>
            ...
            <motion.div variants={fadeUp} className="mb-10 w-full max-w-2xl">
              <h3 className="text-xl font-bold text-slate-900 mb-2 font-cairo">
                {isRTL ? 'فلسفة د. سالي' : 'Dr. Sally\'s Philosophy'}
              </h3>
              <p className="text-lg text-amber-700 font-bold mb-2 font-cairo">
                {isRTL ? 'التشخيص أولاً .. والإجراء ثانياً' : 'Diagnosis First .. Procedure Second'}
              </p>
              <p className="text-slate-500 leading-relaxed font-medium">
                {isRTL 
                  ...
                  : 'Not every case requires the same procedure. That\'s why the process always begins with understanding the case and accurately determining its needs before selecting any treatment step.'}
              </p>
            </motion.div>
            ...
            <motion.div variants={fadeUp} className="mt-12 pt-8 border-t border-slate-200/60 w-full max-w-2xl">
               <p className="text-2xl text-slate-400 italic font-cairo leading-relaxed">
                  "{isRTL ? 'هدفي إن النتيجة تكون مناسبة ليك، مش مجرد نتيجة حلوة.' : 'My goal is for the result to be right for you, not just a beautiful result.'}"
               </p>
            </motion.div>
